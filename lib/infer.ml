...
let rec inferType' check (ctx : typeCtx) (e : Debruijn.expr) :
  ...
  | Fun f ->
      ...
      inferType' check newCtx f
      >>= fun (s, t) -> return (s, Fun (applySubstToMonoType s (FreshVar x), t))
  | App (f, g) ->
      inferType' check ctx f
      >>= fun (s1, tf) ->
      let newCtx = applySubstToCtx s1 ctx in
      inferType' check newCtx g
      >>= fun (s2, tg) ->
      freshName
      >>= fun x ->
      ...
      return
        (combineSubst s3 (combineSubst s2 s1), applySubstToMonoType s3 (FreshVar x))
  | Pair (e1, e2) ->
      inferType' check ctx e1
      >>= fun (s1, t1) ->
      let newCtx = applySubstToCtx s1 ctx in
      inferType' check newCtx e2
      >>= fun (s2, t2) ->
      let s3 = combineSubst s2 s1 in
      return (s3, Pair (applySubstToMonoType s3 t1, applySubstToMonoType s3 t2))
  | (Fst e | Snd e) as e1 ->
      inferType' check ctx e
      >>= fun (s1, t1) ->
      freshName
      >>= fun x ->
      freshName
      ...
      return
        ( s3
        , applySubstToMonoType s3 (match e1 with Fst _ -> FreshVar x | _ -> FreshVar y) )
  | TypeApp (e, t) ->
      inferType' check ctx e
      >>= fun (s1, t1) ->
      freshName
